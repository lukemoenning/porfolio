...
export class Game {
  ...
  startGame() {
    ...
    this.generateAsteroidInterval = setInterval(
      this.generateAsteroid.bind(this),
      800
    ) // starts generating asteroids at 1 every 800 ms
  ...
  buildWordBank() {
    ...
    database.work.forEach((workExperience) => {
      ...
      this.splitSentenceIntoWords(workExperience.location).forEach((word) => {
        wordBank.push(word)
      })
      this.splitSentenceIntoWords(workExperience.date).forEach((word) => {
        wordBank.push(word)
      })
      workExperience.bulletPoints.forEach((bulletPoints) => {
        this.splitSentenceIntoWords(bulletPoints).forEach((word) => {
          wordBank.push(word)
        })
      })
    })
    ...
    database.education.forEach((educationExperience) => {
      ...
      this.splitSentenceIntoWords(educationExperience.location).forEach(
        (word) => {
          wordBank.push(word)
        }
      )
      this.splitSentenceIntoWords(educationExperience.date).forEach((word) => {
        wordBank.push(word)
      })
    ...
    database.projects.forEach((project) => {
      ...
      this.splitSentenceIntoWords(project.description).forEach((word) => {
        wordBank.push(word)
      })
      project.techstack.forEach((technology) => {
        this.splitSentenceIntoWords(technology).forEach((word) => {
          wordBank.push(word)
        })
      })
    })
  ...
  splitSentenceIntoWords(sentence) {
    ...
    while (index < sentenceSplit.length) {
      const randomNum = this.randomNumOneThroughThree()
      const oneThroughThreeWords = sentenceSplit.slice(index, index + randomNum)
...
class Player extends GameObject {
  ...
  initializeEventListeners() {
    ...
    document.addEventListener('keyup', (e) => {
      ...
      if (
        (keyReleased === 'ArrowLeft' || keyReleased === 'KeyA') &&
        this.dx === -this.playerSpeed
      ) {
        this.dx = 0
      }
...
class Asteroid extends GameObject {
  ...
  generateAsteroidScore() {
    const length = this.word.split(' ').length
